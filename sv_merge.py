...
def load_input_file(filename):
    ...
    for row in rows:
        for lib, prot, tool in itertools.product(row['libraries'].split(';'), row['protocol'].split(';'), row['tool_version'].split(';')):
            label = {'library': lib, 'protocol': prot, 'tool_version': tool, 'input_file': filename}
            ...
            try:
                if key not in breakpoints:
                    breakpoints[key] = set()
                bpp = BreakpointPair(b1, b2, flags=[] if FLAGS.LQ not in row.get('filters', '').split(';') else [FLAGS.LQ])
                bpp.label = label
...
def output_cluster_files(clusters, folder, bams_by_lib):
    header = [
            'cluster_id',
            'cluster_size',
            'break1_chromosome',
...
def main():
    ...
    parser.add_argument('-n', '--inputs', help='path to the input files', required=True, action='append')
    ...
    mkdirp(os.path.join(args.output, 'clustering'))
    ...
    with open(temp, 'w') as fh:
        ...
        fh.write('## {0} v{1} {2}\n'.format(__prog__, __version__, datetime.datetime.now()))
        fh.write('## this file details the inputs and the cluster ids they were assigned to\n')
        fh.write('#' + '\t'.join(assignment_header) + '\n')
        for key, bpp_list in bpp_by_library.items():
            libname, protocol = key
            print('for', key, 'there are', len(bpp_list), 'input breakpoint pairs')
            ...
            print('after clustering there are', len(clusters), 'quality breakpoint pairs and', initial_count, 'total pairs')
            ...
            for bpp in bpp_list:
                ...
                for tag, value in bpp.label.items():
                    if tag in row:
                        warnings.warn('label tag {0}={1} is dropped b/c conflicts with header'.format(tag. value))
                    else:
                        row[tag] = value
                fh.write('\t'.join(str(row[k]) for k in assignment_header) + '\n')
            
            for c in clusters:
                tools = ';'.join(sorted(list(set([ k.label['tool_version']  for k in clusters[c] ]))))
                row = {
                        'cluster_id': c.label,
    ...
    for lib, cluster_rows in cluster_rows_by_lib.items():
        ...
        print('splitting', len(cluster_rows), 'clusters into', len(cluster_rows)//JOB_SIZE, 'jobs of size', JOB_SIZE)
        # split the clusters by chromosomes
        cluster_rows.sort(key=lambda x: (x['break1_chromosome'], x['break2_chromosome']))
        ...
        while index < len(cluster_rows):
            ...
            filename = os.path.abspath(os.path.join(args.output, 'clustering/{0}-{1}.tsv'.format(lib, fileno)))
            ...
            output_file = os.path.abspath(os.path.join(args.output, 'validation/{0}-{1}.tsv'.format(lib, fileno)))
            qsub.write(
                    'jid=$(qsub -b sv_validate.py -n {0} -o {1} -b {2} -l {3} -N sv_validate -terse)\n'.format(
                filename, output_file, BAM_FILE_ARGS[lib], lib)
                    + 'VALIDATION_JOBS+=("$jid")\n'
                    )
