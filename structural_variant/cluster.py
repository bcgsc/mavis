...
def redundant_maximal_kcliques(G, **kwargs):
    ...
    if k >= 20:
        warnings.warn(
            'k >= 20 is not recommended as the number of combinations increases exponentially')
...
def cluster_breakpoints(input_pairs, **kwargs):
    ...
    for pair in input_pairs:
        ...
        interval_pair = (Interval(pair.break1.start, pair.break1.end),
                         Interval(pair.break2.start, pair.break2.end))
        interval_key = (interval_pair[0].start, interval_pair[
                        0].end, interval_pair[1].start, interval_pair[1].end)

        for chr1, chr2, o1, o2, s1, s2 in itertools.product(
                [pair.break1.chr],
                [pair.break2.chr],
                [pair.break1.orient] if pair.break1.orient != ORIENT.NS else [
                    ORIENT.LEFT, ORIENT.RIGHT],
                [pair.break2.orient] if pair.break2.orient != ORIENT.NS else [
                    ORIENT.LEFT, ORIENT.RIGHT],
                [pair.break1.strand] if pair.break1.strand != STRAND.NS else [
                    STRAND.POS, STRAND.NEG],
                [pair.break2.strand] if pair.break2.strand != STRAND.NS else [
                    STRAND.POS, STRAND.NEG]
        ):
    ...
    for key, group in sorted(classify.items()):
        ...
        complete_subgraphs = redundant_maximal_kcliques(
            G, k=k)  # every node will be present

        # before we group the subgraphs, should choose the best fit for each
        # node in the subgraphs
        new_sets = Interval.redundant_ordered_hierarchical_clustering(
            complete_subgraphs, r=r)
        # now using the complete subgraphs as our inputs, do ordered
        ...
        for clique in new_sets:
            ...
            events += 1
            size = len(clique)
            # add the cluster breakpoint as the key with a list of the
            # breakpoints used to create it
        ...
        for node, freq in participants.items():
            if freq > 1:
                warnings.warn('node ' + node +
                              ' participates in ' + str(freq) + ' clusters')

    # now for each cluster, merge clusters with the same location and
    # equivalent strand information
    merged_clusters = {}
    for centroid, cluster_set in clusters.items():
        ...
        merged = False
        has_stranded = True if sum(
            [1 if pair.stranded else 0 for pair in cluster_set]) > 0 else False
        for cluster, cset in merged_clusters.get(grouping_key, []):
            curr_has_stranded = True if sum(
                [1 if pair.stranded else 0 for pair in cset]) > 0 else False
